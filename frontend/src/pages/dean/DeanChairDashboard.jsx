...
const DeanChairDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const statistics = {
        total: papers.length,
        pending: papers.filter(p => p.status === pendingStatus).length,
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      pending_dean: 'Awaiting Dean Review',
      pending_program_chair: 'Awaiting Chair Review',
  ...
  const getStatusColor = (status) => {
    const colors = {
      pending_dean: 'text-violet-700 bg-violet-50',
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] animate-fadeIn">
        <div className="relative">
          ...
          <div className={`absolute top-0 left-0 w-16 h-16 border-4 ${isDean ? 'border-violet-500' : 'border-teal-500'} border-t-transparent rounded-full animate-spin`}></div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 animate-fadeIn">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${accentFrom} ${accentTo} flex items-center justify-center shadow-lg`}>
              {isDean ? <Award size={22} className="text-white" /> : <Users size={22} className="text-white" />}
            </div>
            <div>
              <h1 className="text-2xl font-bold text-slate-900">
                {isDean ? 'Dean' : 'Program Chair'} Dashboard
              </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          <div className={`bg-gradient-to-br ${isDean ? 'from-violet-50 to-purple-50 border-violet-200' : 'from-teal-50 to-cyan-50 border-teal-200'} rounded-2xl border-2 p-6 shadow-sm hover:shadow-md transition-shadow`}>
            <div className="flex items-start justify-between">
              <div>
                <p className={`text-sm font-semibold mb-1 ${isDean ? 'text-violet-700' : 'text-teal-700'}`}>Awaiting Review</p>
                <p className="text-4xl font-black text-slate-900">{stats.pending}</p>
              </div>
              <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${accentFrom} ${accentTo} flex items-center justify-center`}>
                <Clock size={20} className="text-white" />
              </div>
            </div>
            <p className={`text-xs mt-3 ${isDean ? 'text-violet-600' : 'text-teal-600'}`}>Requires your approval</p>
          </div>
        ...
        {reviewedPapers > 0 && (
          <div className={`bg-gradient-to-r ${isDean ? 'from-violet-600 to-purple-600' : 'from-teal-600 to-cyan-600'} rounded-2xl p-6 text-white mb-10 shadow-lg`}>
            <div className="flex items-center justify-between">
              ...
              <Award size={60} className="text-white/20" />
            </div>
          </div>
        )}
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className={`text-sm font-semibold ${isDean ? 'text-violet-600 hover:text-violet-800' : 'text-teal-600 hover:text-teal-800'} flex items-center gap-1 transition-colors`}
            >
          ...
          {recentPapers.length === 0 ? (
            ...
            <div className="space-y-4">
              {recentPapers.map(paper => (
                ...
                >
                  <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${accentFrom} ${accentTo} flex items-center justify-center flex-shrink-0`}>
...
export default DeanChairDashboard;
